...
IMAGE_NAME = sebastianhutter/self-service-password
SELFSERVICEPASSWORD_VERSION = "1.0"
...
build_commit:
	docker build --build-arg SELFSERVICEPASSWORD_VERSION=$(SELFSERVICEPASSWORD_VERSION) -t $(IMAGE_NAME):$(COMMIT_ID) .
...
build_latest:
	docker build --build-arg SELFSERVICEPASSWORD_VERSION=$(SELFSERVICEPASSWORD_VERSION) -t $(IMAGE_NAME):latest .
